...
$gambar = (!empty($buku['gambar']) && file_exists($_SERVER['DOCUMENT_ROOT'] . $gambar_path)) ? $gambar_path : $default_gambar;
...
<body>
    ...
    <main>
        ...
        <section class="detail-buku">
            <img class="gambar-buku" src="<?= $gambar ?>" alt="<?= htmlspecialchars($buku['judul']) ?>" width="200">
            <div class="informasi-buku">
                ...
                <p><strong>Tipe Buku:</strong> <?= htmlspecialchars($buku['tipe_buku']) ?></p>
                <p><strong>Status:</strong> <?= htmlspecialchars($buku['status']) ?></p>
                <p><strong>Deskripsi:</strong> <?= nl2br(htmlspecialchars($buku['deskripsi'])) ?></p>
            </div>
        </section>
        ...
        <div class="action-buttons">
            <?php if ($buku['status'] === 'tersedia'): ?>
                <?php if ($buku['tipe_buku'] === 'Buku Elektronik' && $buku['file_path']): ?>
                    <a href="/CODINGAN/3-landingpageuser/layanan/sirkulasi/formpinjam/view_pdf.php?file=<?= urlencode(basename($buku['file_path'])) ?>" target="_blank" class="btn-pinjam">Lihat Sekarang</a>
                <?php else: ?>
